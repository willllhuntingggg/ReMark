...
const ReMarkStorage = {
  ...
  async init() {
    ...
    } else if (settings.defaultColor === this.LEGACY_DEFAULT_AMBER) {
      await this.set(this.KEYS.SETTINGS, { ...settings, defaultColor: this.BRAND_COLOR });
    }
  ...
  async addClip(clipData) {
    ...
    const newClip = {
      ...
      note: clipData.note || '',
      color: clipData.color || this.BRAND_COLOR,
      createdAt: Date.now()
    };

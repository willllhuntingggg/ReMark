...
const ReMarkStorage = {
  ...
  async getOnboardingStatus() {
    ...
    if (['not_started', 'completed', 'dismissed'].includes(status)) return status;
    return settings?.onboardingSeen ? 'dismissed' : 'not_started';
  },

  async setOnboardingStatus(status) {
    if (!['not_started', 'completed', 'dismissed'].includes(status)) {

...
const ReMarkStorage = {
  BRAND_COLOR: '#FF5500',
  LEGACY_DEFAULT_AMBER: '#FFE066',
  MARK_COLOR_PRESETS: {
    trail_orange: '#FF5500',
    blazing_amber: '#F5A623',
    signal_coral: '#FF5C5C',
    atlas_blue: '#3873B2',
    beacon_yellow: '#F5C542',
    harbor_mist: '#468282'
  },
  KEYS: {
    CLIPS: 'markit_clips',
    ...
    UNDO: 'markit_undo',
    AUTH_SESSION: 'remark_auth_session',
    CLOUD_BACKUP_META: 'remark_cloud_backup_meta'
  },
  ...
  DEFAULT_SETTINGS: {
    ...
    quickHighlightModifier: true,
    showMarkPill: true
  },

  normalizeMarkColor(color) {
    const normalized = String(color || '').toUpperCase();
    return Object.values(this.MARK_COLOR_PRESETS).includes(normalized)
      ? normalized
      : this.BRAND_COLOR;
  },
  ...
  async init() {
    ...
    if (!settings) {
      ...
      const defaultColor = this.normalizeMarkColor(settings.defaultColor);
      if (settings.defaultColor !== defaultColor) {
        await this.set(this.KEYS.SETTINGS, { ...settings, defaultColor });
      }
    }
    ...
    if (!clips) {
      ...
      const migratedClips = clips.map((clip) => (
        clip.color === this.LEGACY_DEFAULT_AMBER ? { ...clip, color: this.BRAND_COLOR } : clip
      ));
      if (migratedClips.some((clip, index) => clip !== clips[index])) {
        await this.set(this.KEYS.CLIPS, migratedClips);
      }
    }
  },
  ...
  async addClip(clipData) {
    ...
    const newClip = {
      ...
      postUrl: clipData.postUrl || null,
      feedUrl: clipData.feedUrl || null,
      ...
      note: clipData.note || '',
      color: this.normalizeMarkColor(clipData.color),
      createdAt: Date.now()
    };
  ...
  async getSettings() {
    ...
    return { ...merged, defaultColor: this.normalizeMarkColor(merged.defaultColor) };
  },
  ...
  async updateSettings(newSettings) {
    ...
    updated.defaultColor = this.normalizeMarkColor(updated.defaultColor);
  ...
  async getAuthSession() {
    return await this.get(this.KEYS.AUTH_SESSION);
  },

  async setAuthSession(session) {
    if (!session) {
      if (typeof chrome !== 'undefined' && chrome.storage?.local) {
        await new Promise((resolve) => chrome.storage.local.remove([this.KEYS.AUTH_SESSION], resolve));
      } else if (typeof localStorage !== 'undefined') {
        localStorage.removeItem(this.KEYS.AUTH_SESSION);
      }
      return true;
    }
    return await this.set(this.KEYS.AUTH_SESSION, session);
  },

  async getCloudBackupMeta() {
    return (await this.get(this.KEYS.CLOUD_BACKUP_META)) || {
      lastBackupTime: null,
      status: 'idle'
    };
  },

  async setCloudBackupMeta(meta) {
    const current = await this.getCloudBackupMeta();
    const updated = { ...current, ...meta };
    await this.set(this.KEYS.CLOUD_BACKUP_META, updated);
    return updated;
  }
};

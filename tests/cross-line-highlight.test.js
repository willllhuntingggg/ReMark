...
assert.match(css, /mark\.remark-highlight-mark \{[\s\S]*?padding: 0;/);
...
assert.match(css, /mark\.remark-highlight-mark \{[\s\S]*?box-shadow: none;/);
assert.match(css, /background-image: linear-gradient\([\s\S]*?transparent \.04em,[\s\S]*?var\(--remark-highlight-bg\) \.04em/);
assert.match(css, /border-radius: \.2em;/);
assert.doesNotMatch(css.slice(0, 1500), /::selection \{/);
assert.doesNotMatch(css.slice(0, 1500), /::-moz-selection \{/);

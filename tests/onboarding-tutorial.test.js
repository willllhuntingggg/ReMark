...
assert.match(content, /msg\.action === 'SHOW_FIRST_USE_ONBOARDING'/);
assert.match(content, /void showFirstUseGuide\(\)\.then\(\(shown\) =>/);
assert.match(content, /return true;\s*\}\s*else if \(msg\.action === 'REPLAY_ONBOARDING'/);
...
assert.match(sidepanel, /action: 'SHOW_FIRST_USE_ONBOARDING'/);
assert.match(sidepanel, /REPLAY_CONTENT_SCRIPT_FILES = \['lib\/i18n\.js', 'lib\/storage\.js', 'content\/content\.js'\]/);
assert.match(sidepanel, /CONTENT_STYLE_FILE = 'content\/content\.css'/);
assert.match(sidepanel, /isReplayReceiverUnavailable/);
assert.match(sidepanel, /chrome\.scripting\.executeScript/);
assert.match(sidepanel, /chrome\.scripting\.insertCSS/);
assert.match(sidepanel, /for \(const delay of \[0, 120, 360\]\)/);
assert.match(sidepanel, /result\?\.shown === false/);
assert.match(sidepanel, /async function ensureFirstUseOnCurrentPage\(\)/);
assert.match(sidepanel, /if \(await ReMarkStorage\.getOnboardingStatus\(\) !== 'not_started'\) return false/);
assert.match(sidepanel, /void ensureFirstUseOnCurrentPage\(\)/);

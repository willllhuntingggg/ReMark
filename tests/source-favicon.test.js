...
assert.match(script, /<span class="mark-source" title="\$\{esc\(item\.url\)\}"[\s\S]*sourceIcon/);
// The source title is context only — the whole card is the jump target now.
assert.doesNotMatch(script, /data-action="source"/);
assert.doesNotMatch(script, /mark-source-arrow/);
...
assert.match(css, /\.mark-source \{[\s\S]*?color: var\(--ink-4\);/);
assert.doesNotMatch(css, /mark-source-arrow/);
assert.doesNotMatch(css, /is-glyph-hover/);
